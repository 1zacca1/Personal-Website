...
function stripTags(html) {
  return html
    .replace(/<a[^>]*href=['"]([^'"]+)['"][^>]*>(.*?)<\/a>/gi, '$2') // keep link text
    .replace(/<[^>]+>/g, '')
    .replace(/&amp;/g, '&')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
...
function parseSpinoffs(html) {
  ...
  // Strategy 1 — standard <table> elements
  ...
  while ((tableMatch = tableRe.exec(html)) !== null) {
    ...
    // ── Extract headers (prefer <th>, fall back to first <tr><td>) ──
    let headers = [];
  ...
  // ── Filter to 2026 entries (keep entries with no parseable date too) ──
  return results.filter((s) => {
    const d = s.date || '';
    // Keep if year is 2026, or if we can't parse a year at all
    if (!d) return true;
...
function normalizeEntry(raw, headers, cells, hrefs) {
  ...
  // Positional fallback if normalization failed
  if (!result.parent && !result.spinoff && cells.length >= 2) {
    // Guess: first cell is date-like or company, second is company
    const first = cells[0] || '';
  ...
  // Must have at least parent or spinoff company name
